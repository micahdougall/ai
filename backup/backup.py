...
#         print(f"Stack: {grid.stack}")
#         print(f"Route: {grid.route}")
#
#         # Update states
#         if not percept:
#             print(f"No percept so adding options to safe: {grid.current.options}")
...
#         # Would need to add a target to the grid to work back through the route
#
#         # Else pick a random valid adjacent square
#         printc("Picking a random option from available options.")
#         if not grid.current.options:
#             printc("No option possible")
#             printc(f"Route taken: {grid.route}")
#             printc(f"Stack: {grid.stack}")
#             printc(f"Endpoint: {grid.current.coords}")
#             exit(-1)
#         else:
